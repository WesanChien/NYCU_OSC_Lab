...
static void flush_dcache(void* addr, unsigned long len) {
    ...
    /*
     * QEMU 6.2 does not support Zicbom / cbo.flush.
     * For this ex52 ramfb exercise, a memory barrier is enough.
     */
    __sync_synchronize();
}

/* 
 * Initialize the video framebuffer by writing the configuration to the QEMU
 * 讓 QEMU 知道 FrameBuffer 在哪(etc/ramfb 這個 entry)
 */
void video_init() {
    struct RAMFBCfg cfg = {

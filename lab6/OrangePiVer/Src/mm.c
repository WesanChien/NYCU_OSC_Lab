...
/* ---------- small helpers ---------- */
/*
 * allocator 對外回傳的是 kernel higher-half VA。
 *
 * 但 buddy system 內部永遠以 physical address 管理 frame。
 *
 * 這個 helper 讓轉換期間也能接受：
 *
 *   higher-half VA -> PA
 *   physical addr  -> 그대로 PA
 */
static unsigned long kernel_ptr_to_phys(unsigned long addr) {
    if (addr >= KERNEL_VA_OFFSET)
        return virt_to_phys_addr(addr);
...
void *alloc_pages(unsigned int order) {
    ...
    for (unsigned int current_order = order; current_order <= MAX_ORDER; current_order++) {
        if (!list_empty(&free_area[current_order])) {
            ...
            unsigned long pa = frame_addr(frame_index(page));
            unsigned long va = phys_to_virt_addr(pa);

            uart_puts("[Page] Allocate PA ");
            uart_hex(pa);
...
void free_pages(void *addr) {
    ...
    unsigned long a = kernel_ptr_to_phys(va);
...
static void refill_chunk_pool(int pool) { // 當某 chunk pool 沒有 free chunk 時，要向 Buddy System 要一個 page，然後切成 chunks
    ...
    unsigned long page_pa = kernel_ptr_to_phys((unsigned long)page_addr);

    unsigned long page_idx = addr_to_frame_index(page_pa);
...
void free(void *ptr) {
    ...
    unsigned long pa = kernel_ptr_to_phys(va);

    if (!addr_in_managed_range(pa)) {
        uart_puts("[Free] Ignore pointer outside managed range\n");
        return;
    }

    unsigned long page_idx = addr_to_frame_index(pa);
...
static void *startup_alloc(unsigned long size, unsigned long align) {
    ...
    while (addr + size <= startup_end) {
        ...
        if (!conflict) {
            ...
            return (void *)phys_to_virt_addr(addr);
        }
    }
...
void mm_init(const void *fdt, unsigned long initrd_start, unsigned long initrd_end) {
     ...
     */
    unsigned long dtb_start = kernel_ptr_to_phys((unsigned long)fdt);

    unsigned long dtb_size = fdt_totalsize(fdt);

    early_reserve_add(dtb_start, dtb_size, "DTB");

    unsigned long kernel_start = kernel_ptr_to_phys((unsigned long)__kernel_start);
    unsigned long kernel_end = kernel_ptr_to_phys((unsigned long)__kernel_end);
     ...
     */
    early_reserve_add(kernel_ptr_to_phys((unsigned long)frame_array), frame_array_size, "FrameArray");
